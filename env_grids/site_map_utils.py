...
def validate_workers(workers=1):
    try:
        value = int(workers)
...
def effective_grid_workers(workers=1):
    requested = validate_workers(workers)
    cpu_count = os.cpu_count() or 1
    return max(1, min(requested, cpu_count))


@contextmanager
def grid_worker_context(workers=1):
    effective = effective_grid_workers(workers)
    setter = getattr(grid_maps, "set_num_threads", None)
    getter = getattr(grid_maps, "get_num_threads", None)
...
def make_protein_and_solvent_masks(
    ...
    with grid_worker_context(workers):
...
def _widest_path_cpp(r_local, solvent_mask, seed_mask, connectivity=6, power=4, workers=1):
