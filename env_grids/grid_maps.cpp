...
void set_num_threads_cpp(int workers) {
    if (workers < 1) {
...
py::tuple make_protein_and_solvent_masks_cpp(
    ...
    const std::size_t total = static_cast<std::size_t>(nz) *
...

    {
        ...
        if (n_threads > 1 && n_atoms > 1) {
            std::vector<std::vector<std::uint8_t>> local_masks(
                static_cast<std::size_t>(n_threads),
                std::vector<std::uint8_t>(total, 0)
            );
            ...
            #pragma omp parallel
            {
                ...
                auto &local = local_masks[static_cast<std::size_t>(tid)];
                ...
                for (py::ssize_t iy = 0; iy < ny; ++iy) {
                    for (py::ssize_t ix = 0; ix < nx; ++ix) {
                        ...
                        for (int tid = 0; tid < n_threads; ++tid) {
                            pval = static_cast<std::uint8_t>(
                                pval | local_masks[static_cast<std::size_t>(tid)][idx]
                            );
                        }
                        pm(iz, iy, ix) = pval;
            ...
            for (py::ssize_t iz = 0; iz < nz; ++iz) {
                for (py::ssize_t iy = 0; iy < ny; ++iy) {
                    for (py::ssize_t ix = 0; ix < nx; ++ix) {
                        sm(iz, iy, ix) = static_cast<std::uint8_t>(1 - pm(iz, iy, ix));
                    }
                }
            }
        }
...
py::array_t<float> propagate_logp_exp_decay_cpp(
    ...
    py::array_t<uint8_t> sasa_mask_arr({1, 1, 1});
    bool has_sasa_mask = false;
    if (!sasa_mask_obj.is_none()) {
        sasa_mask_arr = sasa_mask_obj.cast<py::array_t<uint8_t>>();
        if (sasa_mask_arr.ndim() != 3 ||
            sasa_mask_arr.shape(0) != nz ||
            sasa_mask_arr.shape(1) != ny ||
            sasa_mask_arr.shape(2) != nx) {
...

    {
        ...
        if (n_threads > 1 && n_src > 1) {
            std::vector<std::vector<float>> local_fields(
                static_cast<std::size_t>(n_threads),
            ...
            {
                const int tid = omp_get_thread_num();
                auto &local = local_fields[static_cast<std::size_t>(tid)];
                    ...
                    for (int iz = iz_min; iz <= iz_max; ++iz) {
                        ...
                        for (int iy = iy_min; iy <= iy_max; ++iy) {
                            ...
                            for (int ix = ix_min; ix <= ix_max; ++ix) {
                                ...
                                local[idx] += static_cast<float>(v * std::exp(-dist));
                            }
                        }
                    }
                }
            ...
            for (py::ssize_t z = 0; z < nz; ++z) {
                for (py::ssize_t y = 0; y < ny; ++y) {
                    for (py::ssize_t x = 0; x < nx; ++x) {
                        ...
                        float value = 0.0f;
                        for (int tid = 0; tid < n_threads; ++tid) {
                            value += local_fields[static_cast<std::size_t>(tid)][idx];
                        }
                        if (has_sasa_mask && !static_cast<bool>(sm(z, y, x))) {
                            value = 0.0f;
                        }
                        fld(z, y, x) = value;
                    }
                ...
                for (int iz = iz_min; iz <= iz_max; ++iz) {
                    ...
                    for (int iy = iy_min; iy <= iy_max; ++iy) {
                        ...
                        for (int ix = ix_min; ix <= ix_max; ++ix) {
                            ...
                            const double dist = std::sqrt(dist2);
                            fld(iz, iy, ix) += static_cast<float>(v * std::exp(-dist));
                        }
                    }
                }
            }
...
py::array_t<double> compute_electrostatic_grid_cutoff_cpp(
...

    {
        ...
        if (n_threads > 1 && N > 1) {
            std::vector<std::vector<double>> local_grids(
                static_cast<std::size_t>(n_threads),
            ...
            {
                const int tid = omp_get_thread_num();
                auto &local = local_grids[static_cast<std::size_t>(tid)];
            ...
            for (py::ssize_t iz = 0; iz < nz; ++iz) {
                for (py::ssize_t iy = 0; iy < ny; ++iy) {
                    for (py::ssize_t ix = 0; ix < nx; ++ix) {
                        ...
                        double value = 0.0;
                        for (int tid = 0; tid < n_threads; ++tid) {
                            value += local_grids[static_cast<std::size_t>(tid)][idx];
                        }
                        g(iz, iy, ix) = value;
                    }
                }
...
PYBIND11_MODULE(grid_maps, m) {
    ...
    m.def("set_num_threads", &set_num_threads_cpp,
          "Set the maximum OpenMP thread count for grid kernels",
          py::arg("workers"));
    m.def("get_num_threads", &get_num_threads_cpp,
          "Return the current maximum OpenMP thread count for grid kernels");
